...
def generator(x, is_training, num_block, scope_name, reuse, is_gray=True):
  ...
  with tf.variable_scope(scope_name, reuse=reuse) as vscope:
    x = tf.layers.conv2d(x, 30, kernel_size=3, strides=1, padding='SAME',
        activation=tf.nn.relu) # H, W
    x = tf.layers.batch_normalization(x, training=is_training)
    x = tf.layers.conv2d(x, 30, kernel_size=3, strides=2, padding='SAME',
        activation=tf.nn.relu) # H/2, W/2
    x = tf.layers.batch_normalization(x, training=is_training)
    x = tf.layers.conv2d(x, 60, kernel_size=3, strides=2, padding='SAME',
    ...
    x = tf.layers.conv2d_transpose(x, 60, kernel_size=3, strides=2, 
        padding='SAME', activation=tf.nn.relu) # H/4, W/4
    x = tf.layers.batch_normalization(x, training=is_training)

    x = tf.layers.conv2d_transpose(x, 30, kernel_size=3, strides=2, 
